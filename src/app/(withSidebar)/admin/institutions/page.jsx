...
export default function InstitutionsPage() {
  ...
  const handleAddInstitution = async (data) => {
    try {
      ...
      toast.success("تمت إضافة المؤسسة", {
        description: `تمت إضافة المؤسسة ${newInstitution.name}`,
      });
    } catch (error) {
      toast.error("فشل في إضافة المؤسسة");
    }
  };
  ...
  const handleDeleteInstitution = async (id) => {
    try {
      ...
      toast.error("فشل في حذف المؤسسة");
      console.log(error);
    }
  };
  ...
  return (
    <div className="w-full mx-auto py-8 px-5 md:px-10" dir="rtl">
      ...
      <div className="flex justify-between items-center mb-6 flex-row-reverse">
        <InstitutionModal onSubmit={handleAddInstitution} />
