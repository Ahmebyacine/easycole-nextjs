...
export async function GET(req) {
  ...
  if (isNaN(year) || year < 2000 || year > 2100) {
    return new Response(JSON.stringify({ message: "Invalid year format" }), {
      status: 400,
    });
  }

  // Get manager's institutions
  const manager = await User.findById(session.user.id)
    .select("institutions")
    .lean();
  const managerInstitutionIds = manager.institutions;

  // Get employees in manager's institutions
  const employeeIds = await User.find({
    role: "employee",
    institutions: { $in: managerInstitutionIds },
  }).distinct("_id");
  ...
  const stats = await Trainee.aggregate([
    {
      $match: {
        ...
        employee: { $exists: true, $in: employeeIds },
      },
    },
    {
      $lookup: {
  ...
  // Get only employees in manager's institutions
